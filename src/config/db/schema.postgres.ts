...
export const parseJob = table(
  'parse_job',
  {
    id: text('id').primaryKey(),
    userId: text('user_id').references(() => user.id, { onDelete: 'set null' }),
    userEmail: text('user_email').notNull().default(''),
    jobId: text('job_id').unique(), // TextIn async job id
    fileName: text('file_name').notNull().default(''),
    fileType: text('file_type').notNull().default(''), // pdf, image
    fileSize: integer('file_size').default(0), // bytes
    fileUrl: text('file_url'), // R2 public URL for preview
  ...
  (table) => [
    index('idx_parse_job_user_status').on(table.userId, table.status),
    index('idx_parse_job_email').on(table.userEmail),
    index('idx_parse_job_job_id').on(table.jobId),
    index('idx_parse_job_created_at').on(table.createdAt),
  ]
);
