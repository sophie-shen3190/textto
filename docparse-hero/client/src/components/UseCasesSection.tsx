/**
 * UseCasesSection.tsx
 * Design Philosophy: Developer Tool + Glassmorphism Light (same as HeroSection)
 * SEO Strategy:
 *   - All content rendered in DOM (no hidden tabs)
 *   - h2 / h3 semantic headings with target keywords
 *   - Keyword badges as visible, indexable text
 *   - section id anchors for deep linking (#pdf-parsing, #resume-parser, etc.)
...
function JsonBlock({ lines }: { lines: Array<{ key: string; value: string; type?: "string" | "number" | "array" | "comment" }> }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-[#F8FAFC] overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-200 bg-white">
        ...
        <span className="text-[11px] text-slate-400 font-mono ml-1">output.json</span>
      </div>
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 bg-slate-50">
        <span className="text-[11px] text-slate-500 font-medium">Extracted Table — CSV Preview</span>
        <span className="ml-auto text-[11px] text-emerald-600 font-medium bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">✓ Parsed</span>
      </div>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                {row.map((cell, j) => (
                  <td key={j} className="px-3 py-2 text-slate-600 font-mono">{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
function MarkdownBlock({ lines }: { lines: Array<{ text: string; type: "h1" | "h2" | "body" | "bold" | "tag" | "divider" }> }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100 bg-slate-50">
        <span className="text-[11px] text-slate-500 font-medium">output.md</span>
        <span className="ml-auto text-[11px] text-blue-600 font-medium bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">Markdown</span>
      </div>
      <div className="p-4 font-mono text-xs leading-6 space-y-1">
        {lines.map(({ text, type }, i) => {
          if (type === "h1") return <div key={i} className="text-slate-900 font-bold text-sm">{text}</div>;
          if (type === "h2") return <div key={i} className="text-slate-800 font-semibold mt-1">{text}</div>;
          if (type === "bold") return <div key={i} className="text-slate-700 font-medium">{text}</div>;
          if (type === "tag") return <span key={i} className="inline-block mr-1.5 px-2 py-0.5 rounded bg-blue-50 text-blue-600 border border-blue-100 text-[10px] font-medium">{text}</span>;
          if (type === "divider") return <div key={i} className="border-t border-slate-100 my-1" />;
    ...
    preview: (
      <MarkdownBlock
        lines={[
          ...
          { text: "", type: "divider" },
          { text: "Webhook →", type: "body" },
          { text: "https://your-app.com/webhook/parse", type: "body" },
          { text: "", type: "divider" },
          { text: "PDF", type: "tag" },
          { text: "DOCX", type: "tag" },
          { text: "XLSX", type: "tag" },
...
function KeywordBadges({ keywords }: { keywords: string[] }) {
  return (
    <div className="flex flex-wrap gap-2 mt-4">
      <Tag className="w-3.5 h-3.5 text-slate-300 mt-0.5 flex-shrink-0" />
      {keywords.map((kw) => (
        <span
          key={kw}
          className="px-2.5 py-1 rounded-md text-[11px] font-medium bg-slate-50 border border-slate-200 text-slate-500 hover:border-blue-200 hover:text-blue-600 hover:bg-blue-50 transition-colors cursor-default"
        >
          {kw}
        </span>
      ))}
...
function UseCaseRow({ useCase, index }: { useCase: UseCase; index: number }) {
  ...
  return (
    <motion.section
      ...
      className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16 items-center py-14 border-b border-slate-100 last:border-0"
    >
      {/* Text side */}
      <div className={`flex flex-col gap-5 ${!isEven ? "lg:order-2" : ""}`}>
        {/* Icon + badge row */}
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl ${useCase.iconBg} flex items-center justify-center ${useCase.iconColor}`}>
            {useCase.icon}
          </div>
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${useCase.badgeBg} ${useCase.badgeText} border border-current/10`}>
            {useCase.badge}
          </span>
        </div>

        {/* Heading — uses target keyword in h3 */}
        ...
        {/* Keyword badges — SEO埋词，视觉低调 */}
        <KeywordBadges keywords={useCase.keywords} />

        {/* CTA link */}
        <a
          href={`/playground?usecase=${useCase.id}`}
          className="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 group w-fit"
        >
  ...
  return (
    <section className="bg-white py-20">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div>
          {USE_CASES.map((uc, i) => (
            <UseCaseRow key={uc.id} useCase={uc} index={i} />
          ))}
        </div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-6 p-8 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 text-white"
        >
          <div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            <a
              href="/playground"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white text-blue-700 text-sm font-semibold hover:bg-blue-50 transition-colors shadow-sm"
            >
